...
class HistoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppTexts.get('history'), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
        ...
        child: ConstrainedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (log['type'] == 'in') { icon = Icons.arrow_downward; color = Colors.green; } 
              else if (log['type'] == 'out') { icon = Icons.arrow_upward; color = Colors.red; } 
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  title: Text('${log['action']}: ${log['item']}', style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text(log['date']),
                  trailing: log['qty'] > 0
                      ? Text('${log['type'] == 'in' ? '+' : '-'}${log['qty']}', style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 18))
                      : null,
                ),
              );
            },
